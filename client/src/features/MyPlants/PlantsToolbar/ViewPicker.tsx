...
const ViewPicker: FC<ViewPickerProps> = (props) => {
  ...
  return (
    <ToggleButtonGroup
      ...
      exclusive
    >
      <ToggleButton value={CardView.GRID} aria-label={i18n.t('view.grid')}>
        <GridView />
      </ToggleButton>
      <ToggleButton value={CardView.CARDS} aria-label={i18n.t('view.cards')}>
        <ViewAgendaOutlined />
      </ToggleButton>
      <ToggleButton value={CardView.ROWS} aria-label={i18n.t('view.rows')}>
        <FormatAlignJustify />
      </ToggleButton>
    </ToggleButtonGroup>
  )
